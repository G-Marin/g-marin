...
/*
  Creates a new node of file or directory type in the File Tree, with 
  path oPPath, parent oNParent, and pvContents, as well as ulLength the 
  byte size of pvContents. Returns an int SUCCESS status and sets 
  *poNResult to be the new node if successful.Otherwise, sets *poNResult 
  to NULL and returns status:
  * MEMORY_ERROR if memory could not be allocated to complete request
  * CONFLICTING_PATH if oNParent's path is not an ancestor of oPPath
  * ROOT_ERROR if file path was not rooted at directory
...
/* Returns the path object representing the absolute path of oNNode. */
Path_T Node_getPath(Node_T oNNode);

/* If oNNode is a file, returns the contents*/
void* Node_getFileContents(Node_T oNNode);

/* If oNNode is a file, change its contents to pvNewContents and 
its ulLength to newulLength*/
void Node_replaceFileContents(Node_T oNNode, void* pvNewContents, size_t newulLength);

/* If oNNode is a file, returns the length of file contents*/
